...
class Float():
    ...
    @staticmethod
    def Read(_bytes: bytes) -> (bytes, float): # type: ignore
...
class VarInt():
    # An integer that is only as long as it needs to be.
    @staticmethod
    ...
    @staticmethod
    def Read(_bytes: bytes) -> (bytes, int): # type: ignore
        b = bytearray(_bytes)
        data = 0
        shift = 0
...
class String():
    # A string, prefixed by a variable length integer.
